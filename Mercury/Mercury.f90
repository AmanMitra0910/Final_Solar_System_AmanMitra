...
program simulate_orbit
    ...
    real :: position(3)
    real :: semi_major_axis, eccentricity, t_mercury
    integer :: i, num_steps
    open(unit=10, file='mercury_positions.dat', status='replace')
    ...
    call initialize_planet(mercury, name, mass, position, radial_velocity, 0.0, semi_major_axis, eccentricity)

    call calculate_orbital_velocity(mercury, central_mass, orbital_velocity)

    call initialize_planet(mercury, name, mass, position, radial_velocity, orbital_velocity, semi_major_axis, eccentricity)
    call print_planet_info(mercury)

    call calculate_time_period(mercury, t_mercury)

    dt = 86400.0 
    total_time = t_mercury 
    num_steps = int(total_time / dt)

    write(10, *) mercury%position(1), mercury%position(2), mercury%position(3)

    do i = 1, num_steps
        call update_position(mercury, dt)
        write(10, *) mercury%position(1), mercury%position(2), mercury%position(3)
    end do
