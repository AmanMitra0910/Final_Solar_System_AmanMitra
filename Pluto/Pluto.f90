...
program simulate_orbit
    ...
    type(PlanetType) :: pluto
    character(len=20) :: name
    real :: mass, central_mass, radial_velocity, orbital_velocity, dt, total_time
    real :: position(3)
    real :: semi_major_axis, eccentricity, t_pluto, incline
    integer :: i, num_steps
    open(unit=10, file='pluto_positions.dat', status='replace')

    name = 'Pluto'
    mass = 1.02413e26
    position = (/5.903e12, 0.0, 0.0/)
    radial_velocity = 2.978e4
    semi_major_axis = 5.903e12  
    eccentricity = 0.0086 
    incline = 17
    central_mass = 1.989e30     

    call initialize_planet(pluto, name, mass, position, radial_velocity, 0.0, semi_major_axis, eccentricity, incline)

    call calculate_orbital_velocity(pluto, central_mass, orbital_velocity)

    call initialize_planet(pluto, name, mass, position, radial_velocity, orbital_velocity, semi_major_axis, eccentricity, incline)
    call print_planet_info(pluto)

    call calculate_time_period(pluto, t_pluto)

    dt = 3456000.0 
    total_time = t_pluto 
    num_steps = int(total_time / dt)

    write(10, *) pluto%position(1), pluto%position(2), pluto%position(3)

    do i = 1, num_steps
        call update_position(pluto, dt)
        write(10, *) pluto%position(1), pluto%position(2), pluto%position(3)
    end do
