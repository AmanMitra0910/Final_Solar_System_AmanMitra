...
program simulate_orbit
    ...
    real :: position(3)
    real :: semi_major_axis, eccentricity, t_mars
    integer :: i, num_steps
    open(unit=10, file='mars_positions.dat', status='replace')
    ...
    call initialize_planet(mars, name, mass, position, radial_velocity, 0.0, semi_major_axis, eccentricity)

    call calculate_orbital_velocity(mars, central_mass, orbital_velocity)

    call initialize_planet(mars, name, mass, position, radial_velocity, orbital_velocity, semi_major_axis, eccentricity)
    call print_planet_info(mars)

    call calculate_time_period(mars, t_mars)

    dt = 86400.0 
    total_time = t_mars 
    num_steps = int(total_time / dt)

    write(10, *) mars%position(1), mars%position(2), mars%position(3)

    do i = 1, num_steps
        call update_position(mars, dt)
        write(10, *) mars%position(1), mars%position(2), mars%position(3)
    end do
