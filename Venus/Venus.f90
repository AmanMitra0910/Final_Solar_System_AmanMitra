...
program simulate_orbit
    ...
    real :: position(3)
    real :: semi_major_axis, eccentricity, t_venus
    integer :: i, num_steps
    open(unit=10, file='venus_positions.dat', status='replace')
    ...
    call initialize_planet(venus, name, mass, position, radial_velocity, 0.0, semi_major_axis, eccentricity)

    call calculate_orbital_velocity(venus, central_mass, orbital_velocity)

    call initialize_planet(venus, name, mass, position, radial_velocity, orbital_velocity, semi_major_axis, eccentricity)
    call print_planet_info(venus)

    call calculate_time_period(venus, t_venus)

    dt = 86400.0 
    total_time = t_venus 
    num_steps = int(total_time / dt)

    write(10, *) venus%position(1), venus%position(2), venus%position(3)

    do i = 1, num_steps
        call update_position(venus, dt)
        write(10, *) venus%position(1), venus%position(2), venus%position(3)
    end do
