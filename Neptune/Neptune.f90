...
program simulate_orbit
    ...
    real :: position(3)
    real :: semi_major_axis, eccentricity, t_neptune
    integer :: i, num_steps
    open(unit=10, file='neptune_positions.dat', status='replace')

    name = 'Neptune'
    mass = 1.02413e26
    position = (/4.495e12, 0.0, 0.0/)
    radial_velocity = 2.978e4
    semi_major_axis = 4.495e12  
    eccentricity = 0.0086       
    central_mass = 1.989e30     

    call initialize_planet(neptune, name, mass, position, radial_velocity, 0.0, semi_major_axis, eccentricity)

    call calculate_orbital_velocity(neptune, central_mass, orbital_velocity)

    call initialize_planet(neptune, name, mass, position, radial_velocity, orbital_velocity, semi_major_axis, eccentricity)
    call print_planet_info(neptune)

    call calculate_time_period(neptune, t_neptune)

    dt = 1728000.0 
    total_time = t_neptune 
    num_steps = int(total_time / dt)

    write(10, *) neptune%position(1), neptune%position(2), neptune%position(3)

    do i = 1, num_steps
        call update_position(neptune, dt)
        write(10, *) neptune%position(1), neptune%position(2), neptune%position(3)
    end do
