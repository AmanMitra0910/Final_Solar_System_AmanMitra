...
program simulate_orbit
    ...
    real :: position(3)
    real :: semi_major_axis, eccentricity, t_earth
    integer :: i, num_steps
    open(unit=10, file='earth_positions.dat', status='replace')
    ...
    call initialize_planet(earth, name, mass, position, radial_velocity, 0.0, semi_major_axis, eccentricity)

    call calculate_orbital_velocity(earth, central_mass, orbital_velocity)

    call initialize_planet(earth, name, mass, position, radial_velocity, orbital_velocity, semi_major_axis, eccentricity)
    call print_planet_info(earth)

    call calculate_time_period(earth, t_earth)

    dt = 86400.0 
    total_time = t_earth 
    num_steps = int(total_time / dt)

    write(10, *) earth%position(1), earth%position(2), earth%position(3)

    do i = 1, num_steps
        call update_position(earth, dt)
        write(10, *) earth%position(1), earth%position(2), earth%position(3)
    end do
