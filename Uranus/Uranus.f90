...
program simulate_orbit
    ...
    real :: position(3)
    real :: semi_major_axis, eccentricity, t_uranus
    integer :: i, num_steps
    open(unit=10, file='uranus_positions.dat', status='replace')

    name = 'Uranus'
    mass = 8.6810e25
    position = (/2.875e12, 0.0, 0.0/)
    radial_velocity = 2.978e4
    semi_major_axis = 2.875e12  
    eccentricity = 0.0472       
    central_mass = 1.989e30     

    call initialize_planet(uranus, name, mass, position, radial_velocity, 0.0, semi_major_axis, eccentricity)

    call calculate_orbital_velocity(uranus, central_mass, orbital_velocity)

    call initialize_planet(uranus, name, mass, position, radial_velocity, orbital_velocity, semi_major_axis, eccentricity)
    call print_planet_info(uranus)

    call calculate_time_period(uranus, t_uranus)

    dt = 864000.0 
    total_time = t_uranus 
    num_steps = int(total_time / dt)

    write(10, *) uranus%position(1), uranus%position(2), uranus%position(3)

    do i = 1, num_steps
        call update_position(uranus, dt)
        write(10, *) uranus%position(1), uranus%position(2), uranus%position(3)
    end do
