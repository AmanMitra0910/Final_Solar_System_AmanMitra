...
program simulate_orbit
    ...
    real :: position(3)
    real :: semi_major_axis, eccentricity, t_saturn
    integer :: i, num_steps
    open(unit=10, file='saturn_positions.dat', status='replace')

    name = 'Saturn'
    mass = 5.6834e26
    position = (/1.433e12, 0.0, 0.0/)
    radial_velocity = 2.978e4
    semi_major_axis = 1.433e12  
    eccentricity = 0.0565       
    central_mass = 1.989e30     

    call initialize_planet(saturn, name, mass, position, radial_velocity, 0.0, semi_major_axis, eccentricity)

    call calculate_orbital_velocity(saturn, central_mass, orbital_velocity)

    call initialize_planet(saturn, name, mass, position, radial_velocity, orbital_velocity, semi_major_axis, eccentricity)
    call print_planet_info(saturn)

    call calculate_time_period(saturn, t_saturn)

    dt = 864000.0 
    total_time = t_saturn 
    num_steps = int(total_time / dt)

    write(10, *) saturn%position(1), saturn%position(2), saturn%position(3)

    do i = 1, num_steps
        call update_position(saturn, dt)
        write(10, *) saturn%position(1), saturn%position(2), saturn%position(3)
    end do
